...
const placeOrder = async (req, res) => {
   ...
   try {
      ...
      const session = await stripe.checkout.sessions.create({
         ...
         mode: "payment",
         success_url: `${frontendUrl}/verify?success=true&orderId=${newOder._id}`,
         cancel_url: `${frontendUrl}/verify?success=false&orderId=${newOder._id}`
      });
...
const verifyOrder = async (req, res) => {
   ...
   try {
      if (success == "true") {
         ...
         return res.status(402).json({ success: false, messgae: "Not Paid" })
      }
   } catch (error) {
      console.log(error)
